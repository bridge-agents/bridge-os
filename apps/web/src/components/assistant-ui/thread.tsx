...
const ThreadRoot: FC<{ isEmpty: boolean }> = ({ isEmpty }) => {
  ...
  return (
    <ThreadPrimitive.Root
      ...
      style={{
        ["--thread-max-width" as string]: "76rem",
        ["--composer-bg" as string]:
          "color-mix(in oklab, var(--color-muted) 45%, var(--color-background))",
    ...
    >
      ...
      >
        <div
          className={cn(
            "mx-auto flex w-full max-w-(--thread-max-width) flex-1 flex-col px-3 pt-4 sm:px-5 lg:px-8",
...
const ComposerAction: FC = () => {
  ...
  return (
    <div className="aui-composer-action-wrapper relative flex min-w-0 items-end justify-between gap-2">
...
const AssistantRunIndicator: FC = () => {
  const isStreaming = useAuiState((s) =>
    s.message.parts.some((part) => part.type === "text" && "text" in part && Boolean(part.text)),
  );
  return (
    <span
      data-slot="aui_assistant-message-indicator"
      className="inline-flex items-center gap-2 py-1 text-sm text-muted-foreground"
      role="status"
      aria-live="polite"
    >
      <DotMatrix
        state={isStreaming ? "streaming" : "thinking"}
        label={isStreaming ? "Streaming response" : "Thinking"}
        className="size-4"
      />
